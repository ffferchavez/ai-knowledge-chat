...
-- Apply after 001_initial.sql. Idempotent: safe to re-run.
-- Existing rows keep document_id set; source_id / source_page_id optional until backfilled.
...
CREATE INDEX IF NOT EXISTS idx_sources_kb ON public.sources (knowledge_base_id);
CREATE INDEX IF NOT EXISTS idx_sources_org ON public.sources (organization_id);
CREATE INDEX IF NOT EXISTS idx_sources_type_status ON public.sources (source_type, status);
...
-- One row per source when linked (matches former UNIQUE on source_id)
CREATE UNIQUE INDEX IF NOT EXISTS documents_source_id_key ON public.documents (source_id);
...
CREATE INDEX IF NOT EXISTS idx_source_pages_org ON public.source_pages (organization_id);

DROP TRIGGER IF EXISTS source_pages_updated_at ON public.source_pages;
...
ALTER TABLE public.document_chunks
  ADD COLUMN IF NOT EXISTS source_page_id uuid REFERENCES public.source_pages (id) ON DELETE CASCADE;

ALTER TABLE public.document_chunks
  ALTER COLUMN document_id DROP NOT NULL;

ALTER TABLE public.document_chunks
  DROP CONSTRAINT IF EXISTS document_chunks_one_parent;
...
CREATE UNIQUE INDEX IF NOT EXISTS document_chunks_page_chunk_idx
  ON public.document_chunks (source_page_id, chunk_index)
  WHERE source_page_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_document_chunks_source_page ON public.document_chunks (source_page_id);
...
CREATE INDEX IF NOT EXISTS idx_ingestion_jobs_status_created ON public.ingestion_jobs (status, created_at);
...
DROP POLICY IF EXISTS "sources_all_member" ON public.sources;
CREATE POLICY "sources_all_member"
  ON public.sources FOR ALL
  USING (public.is_org_member(organization_id))
  WITH CHECK (public.is_org_member(organization_id));

DROP POLICY IF EXISTS "source_pages_all_member" ON public.source_pages;
...
DROP POLICY IF EXISTS "ingestion_jobs_delete_member" ON public.ingestion_jobs;
